...
Controller.prototype.run = function() {
  ...
  $("form").on("submit", function(e) {
    ...
    if (input === "") {
      ...
      controller.view.message_error_validation(
        "Debes completar el campo. No puede estar vacío."
      );
    } else if (input.length > 4) {
      controller.view.add_class_error();
      controller.view.message_error_validation(
        "¡Inválido! No debes ingresar mas de 4 dígitos."
      );
    } else if (input.length < 4) {
      controller.view.add_class_error();
      controller.view.message_error_validation(
        "¡Inválido! No debes ingresar menos de 4 dígitos."
      );
    } else if (!input.match(/^(?:([0-9])(?!.*\1)){4}$/)) {
      controller.view.add_class_error();
      controller.view.message_error_validation("No debes repetir dígitos.");
    } else if (input.length === 4) {
